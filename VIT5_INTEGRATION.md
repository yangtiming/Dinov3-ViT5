...
### 6. [dinov3/configs/train/vit5_base_im1k.yaml](dinov3/configs/train/vit5_base_im1k.yaml)（新增预设）

以 `vitl_im1k_lin834.yaml` 为模板，`student:` 块替换为 ViT-5-Base-SwiGLU 配置：
`arch=vit_base / norm_layer=rmsnorm / ffn_layer=swiglu / ffn_ratio=2.667 /
qkv_bias=ffn_bias=proj_bias=false / layerscale=null / n_storage_tokens=4 /
qk_norm=true / register_rope_enabled=true / register_rope_theta=100 /
drop_path_rate=0.1`。其它训练/优化/评估超参与官方 vitl 预设保持一致（`sqrt_wrt_1024` 自动按总 batch 缩放 lr）。
...
等价于 `models_v2.py::vit5_base_swi`。新增其它变体时拷贝此 yaml，改 `arch / drop_path_rate` 即可：
- `vit5_small*`：`student.arch=vit_small`。
- `vit5_large*`：`student.arch=vit_large`，`drop_path_rate=0.3`。
- `vit5_base`（普通 MLP，带 layer_scale）：`ffn_layer=mlp`、`ffn_ratio=4`、`layerscale=1e-5`。
